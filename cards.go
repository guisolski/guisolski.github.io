...
// Cards renders the info grid: social, contact, education, languages, résumé.
func Cards() app.UI {
	return app.Section().Class("cards").Aria("label", "Details").Body(
		card("Around the web", linkList(socialLinks)),
		card("Contact", linkList(contactLinks)),
		card("Education", linkList(courseLinks)),
		card("Programming languages",
			app.Ul().Class("pills").Body(
				app.Range(programmingLanguages).Slice(func(i int) app.UI {
		...
		card("Languages",
			app.Ul().Class("tags").Body(
				app.Range(spokenLanguages).Slice(func(i int) app.UI {
					l := spokenLanguages[i]
...
// icon returns a small inline stroke SVG by name, so the site needs no icon
// font and makes no external requests.
func icon(name string) app.UI {
	...
	return app.Raw(`<svg class="icon" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">` + p + `</svg>`)
}
